    ...
    <div class="book_borrows">
        <el-row style="margin-top: 20px;">
            <el-col :span="14" :offset="4">
                <header class="form_header"><h1>借阅图书</h1></header>
        ...
        methods: {
            ...
            booking() {
                this.$refs.bookFormRef.validate(async valid => {
                    ...
                    if (window.sessionStorage.getItem('bookable') == 'false') {
                        alert("您的信誉积分过低，请先还清罚金再进行借书活动");
                        return;
                    }
                    let msg = "";
                    let status = 200; 
                    ...
                    let result = await this.$http.post('/apip/api/books/borrow/' + this.bookForm.book_id, {
                        hearders: {
                            'Authorization':Token
                        },
                    })
